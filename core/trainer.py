...
class Trainer:
    ...
    def define_model(self, model_name, **kwargs):
        ...
        # ── Branch ablation variants (FM-rnafm, transcript-grouped split) ──
            ...
            self.model = BSCANUnified(encoder_type='rnafm', use_cached=True,
                                      use_cnn=True, use_stem=True, use_attn=True, **kwargs).to(self.device)
            self.num_inputs = 6
        elif model_name == 'bscan_unified_fm_mlponly':
            # FM + projection + MLP only — no CNN/stem/attn branches
            self.model = BSCANUnified(encoder_type='rnafm', use_cached=True,
                                      use_cnn=False, use_stem=False, use_attn=False, **kwargs).to(self.device)
            self.num_inputs = 6
        elif model_name == 'bscan_unified_fm_nocnn':
            # FM + stem + attn (no CNN branch)
            self.model = BSCANUnified(encoder_type='rnafm', use_cached=True,
                                      use_cnn=False, use_stem=True, use_attn=True, **kwargs).to(self.device)
            self.num_inputs = 6
        elif model_name == 'bscan_unified_fm_nostem':
            # FM + CNN + attn (no stem branch)
            self.model = BSCANUnified(encoder_type='rnafm', use_cached=True,
                                      use_cnn=True, use_stem=False, use_attn=True, **kwargs).to(self.device)
            self.num_inputs = 6
        elif model_name == 'bscan_unified_fm_noattn':
            # FM + CNN + stem (no cross-attention)
            self.model = BSCANUnified(encoder_type='rnafm', use_cached=True,
                                      use_cnn=True, use_stem=True, use_attn=False, **kwargs).to(self.device)
            self.num_inputs = 6
        elif model_name == 'bscan_unified_fm_cnnonly':
            # FM + CNN only
            self.model = BSCANUnified(encoder_type='rnafm', use_cached=True,
                                      use_cnn=True, use_stem=False, use_attn=False, **kwargs).to(self.device)
            self.num_inputs = 6
        elif model_name == 'bscan_unified_fm_stemonly':
            # FM + stem only
            self.model = BSCANUnified(encoder_type='rnafm', use_cached=True,
            ...
            self.model = BSCANUnified(encoder_type='rnafm', use_cached=True,
                                      use_cnn=False, use_stem=False, use_attn=True, **kwargs).to(self.device)
            self.num_inputs = 6
