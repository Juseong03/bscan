...
> 상태: **모든 실험 완료.** Adapter 외부 AUC, 전 모델 Tier2/3, Mamba 5-seed 완료.
...
| BSCAN-FM+Mamba adapter | FM 위에 Mamba SSM 모듈 | ✅ 완료 (4 seeds, seed 1004 이상치 제외) |
...
### 3.5 FM Adapter Ablation: 지역/순차 처리가 일반화를 향상시킨다

| 지표 | BSCAN-RNA-FM | BSCAN-FM+CNN | BSCAN-FM+Mamba |
|------|:------------:|:------------:|:--------------:|
| 내부 AUC | 0.917 | 0.913 | 0.912 |
| 외부 AUC | 0.842 | **0.865** | **0.864** |
| Drop% | 8.2% | **5.3%** | **5.3%** |
| Tier2 AUC | 0.475 | 0.512 | 0.534 |
| Tier3 AUC | 0.496 | 0.496 | 0.498 |

**핵심 발견: 지역(CNN) 또는 순차(Mamba) 처리를 FM 위에 추가하면 외부 일반화가 ~8% → ~5.3%로 일관되게 개선된다.**

해석:
- 순수 FM 임베딩은 전역 컨텍스트 표현 → exon composition bias에 취약 (drop 8%)
- CNN adapter(로컬 k-mer 패턴) + Mamba adapter(순차/위치 패턴)는 FM이 지나치게 학습한 전역 shortcut을 보정
- 두 방식 모두 drop%가 5.3%로 동일 → "지역/순차 귀납 편향(inductive bias) 추가"가 핵심, 구체적 구조는 중요치 않음
- 내부 AUC는 소폭 하락 (0.917→0.913/0.912) — 약간의 train/test 분포 맞춤 능력을 포기하는 대신 일반화를 얻는 tradeoff

Tier2/3에서의 adapter 효과:
- Tier2(LS-intron 구분): FM raw 0.475 → CNN 0.512 → Mamba 0.534 (소폭 개선, 여전히 chance 수준)
- Tier3(BS-intron 구분): 모든 adapter 모델 ~0.498 (exon-dominant 특성은 adapter로 극복 불가)

→ **Adapter가 외부 일반화를 개선하는 메커니즘은 intron specificity 학습이 아닌, FM의 전역 exon 편향 완화에 있다.**
...
| 지역/순차 adapter가 FM 일반화를 향상 | CNN/Mamba adapter drop 5.3% vs FM raw 8% | ✅ **신규** |
| Adapter 효과는 intron specificity 아닌 exon bias 완화 | Tier3 adapter ≈ FM raw, 외부 AUC만 개선 | ✅ **신규** |
...
## 7. ✅ 모든 실험 완료 (2026-06-03)

모든 빈칸이 채워졌습니다. `results/paper_table_master.csv` 참조.

**향후 선택사항:**
- FM hard neg 10-seed expansion (현재 3 seeds, 선택사항)
- Mamba seed 1004 재훈련 (이상치 원인 파악 후 결정)
- Adapter 모델을 transcript split으로 재훈련 (현재 sample split)

---

## 8. 핵심 수치 Quick Reference (최종)

```
내부 AUC (best):              BSCAN-FM 4종 0.916–0.917  (vs CircCNN 0.898, JEDI 0.854)
외부 AUC (best, raw):         BSCAN-RNABERT 0.846
외부 AUC (best, +duplex):     BSCAN-RNABERT+duplex 0.848
외부 AUC (best, adapter):     BSCAN-FM+CNN 0.865  ← adapter가 raw FM 초과
외부 AUC drop (FM raw):       ~8%  (vs baseline 39–45%)
외부 AUC drop (FM+adapter):   ~5.3%  ← 지역/순차 처리 추가 효과
Trainable params (FM):        ~2.0M  (vs CircCNN-double 8.4M)

Tier2 AUC (CNN 계열 best):   BSCAN-base 0.727, CircCNN 0.719, CircCNN-tri 0.715
Tier2 AUC (FM raw):           0.45–0.57  (exon classifier — intron 구분 불가)
Tier2 AUC (FM+adapter):       0.512–0.534  (소폭 개선, 여전히 chance 수준)
Tier3 AUC (std trained):      0.49–0.56  (near-chance)
Tier3 AUC (hnaug one-hot):    0.836–0.843  (hard neg 학습으로 대폭 개선)
Tier3 AUC (hnaug FM):         0.509  (FM은 hnaug에도 불구 돌파 불가)

Duplex gain (외부 AUC):       +0.002–0.003
Bio profile #1:               BSCAN-RNA-FM+duplex (0.942)
A-rich risk (FM):             0.001–0.007  (safe)
A-rich risk (JEDI):           0.251  (危)

BSCAN-base Tier3:             0.535 ± 0.022  (수정: 기존 0.523은 bscan_seq_lite 오기재)
```
